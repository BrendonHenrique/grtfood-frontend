  ...
  <div>
    ...
    <q-dialog v-model="fazerPedido" @show="resetaFormulario" persistent>
      <q-card style="width: 700px; max-width: 80vw;height:100vh;">
        ...
        <q-card-section>
          <div class="row">
            <span class="text-h5">Faça seu pedido</span>
            <q-space />
        ...
        <q-form @submit="confirmarPedido">
          ...
          <q-card-section>
            ...
            <!-- Options -->
            <div
              v-for="(multiplosOptions,idx) in multiplosOptionsGroup"
            ...
            >
              ...
              <q-option-group
                v-model="pedidoMultiplosGroup[idx]"
                :options="multiplosOptions.options"
                color="red"
              />
            </div>
            <!--  -->

            <q-banner v-show="false" class="bg-primary text-white q-mt-sm" dense>Observações</q-banner>
    ...
    <!-- Modais de avisos -->
    <sweet-modal icon="success" ref="sucessModal">Pedido realizado com sucesso!</sweet-modal>

    <sweet-modal icon="warning" ref="warningModal">Pedido cancelado!</sweet-modal>
    <!--  -->
  </div>
</template>
  ...
  methods: {
    ...
    filtrarNome(val, update) {
      ...
      update(() => {
        ...
        this.opcoesDeNomes = listaDeNomes.filter(
